    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages((prev) => [
                ...
                {
                    role: "assistant",
                    content: data.answer || data.response || "I'm having a bit of trouble thinking right now.",
                },
            ]);
        } catch (error) {
            console.log(error);
                ...
                {isOpen && (
                    ...
                    >
                        ...
                        <div className="p-4 bg-zinc-800/80 border-b border-brand/30 flex justify-between items-center">
                            ...
                            {/* X button */}
                            <button onClick={() => setIsOpen(false)} className="text-brand hover:text-red-500 transition-colors">
                                <X size={20} />
                            </button>
                        </div>

                        {/* Chat Messages */}
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-zinc-700" onWheel={(e) => e.stopPropagation()}>
                            {messages.map((msg, i) => (
