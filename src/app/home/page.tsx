...
const HomePage = () => {
    ...
    return (
        <section id="home" className="flex flex-col-reverse md:flex-row md:items-center justify-between ">
            {/* Hero Section */}
            <motion.div initial={{opacity: 0, x: -20}} whileInView={{opacity: 1, x: 0}} transition={{duration: 0.6}} className="mt-2 md:mt-0">
                <motion.p initial={{opacity: 0, y: 10}} animate={{opacity: 1, y: 0}} className="text-brand font-mono mb-4 text-center md:text-start ">
                    Hi, my name is
                </motion.p>
                <motion.h1 initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.1}} className="text-6xl md:text-8xl font-bold mb-6 text-center md:text-start">
                    Shriharsh
                </motion.h1>
                <motion.p className="max-w-xl text-lg opacity-70 mb-8 text-center md:text-start " initial={{opacity: 0}} animate={{opacity: 1}} transition={{delay: 0.2}}>
                    A Full Stack Developer passionate about crafting seamless digital experiences. Always excited to tackle new challenges where I can create value and grow as a coder. Let's connect
                ...
                <div className="flex gap-4 flex-wrap md:p-[2px] mb-8 justify-center md:justify-start ">
                    <a href="#projects" className="bg-brand text-black px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform cursor-pointer">
                        My Projects
                    </a>
                    <a href="#contact" className="border border-brand/30 px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform cursor-pointer">
                        Let's Connect
                    </a>
                </div>
            ...
            {/* Images */}
            <motion.div initial={{opacity: 0, x: 20}} whileInView={{opacity: 1, x: 0}} transition={{duration: 0.6, delay: 0.2}} className="hidden md:flex ">
                {/* Desktop Image */}
                <div className="relative flex items-center justify-center">
                    <div className="absolute -z-12 w-85 h-98 bg-[#22c55e] rounded-full mix-blend-screen filter blur-[80px] opacity-30"></div>
                    <Image src="/images/ShriharshImageWBG.avif" priority alt="N/A" width={500} height={500} loading="eager" className="w-full h-auto" style={{height: "auto"}} />
                </div>
            </motion.div>
            <div className="flex items-center justify-center p-3 md:hidden">
                <Image src="/images/Shriharsh.avif" priority alt="N/A" width={300} height={500} className="w-full h-auto rounded-2xl" style={{height: "auto", width: "auto"}} />
            </div>
        </section>
    );
};
